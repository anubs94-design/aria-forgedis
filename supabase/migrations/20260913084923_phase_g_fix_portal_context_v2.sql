-- get_portal_context v3 : Industrial via dirigeant_id ou salaries.user_id
-- Suppression de la jointure par email invalide (sal.email inexistant)
...
BEGIN
  ...
  BEGIN
    SELECT jsonb_agg(jsonb_build_object(
      'product', pe.product, 'plan', pe.plan, 'status', pe.status,
      'ends_at', pe.ends_at, 'source', 'entreprise'))
    INTO v_ind
    FROM public.entreprises ent
    JOIN public.product_entitlements pe ON pe.entreprise_id = ent.id
    WHERE (
      ent.dirigeant_id = v_uid
      OR EXISTS (
        SELECT 1 FROM public.salaries sal
        WHERE sal.entreprise_id = ent.id
          AND sal.user_id = v_uid
          AND sal.actif = true
      )
    )
      AND pe.status IN ('trialing','active')
  ...
  FROM public.clients WHERE email = v_email AND actif = true LIMIT 1;
