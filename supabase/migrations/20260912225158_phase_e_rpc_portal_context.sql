-- phase_e_rpc_portal_context + fixes (version finale v3)
-- get_portal_context : user_id direct, Industrial via dirigeant_id ou salaries.user_id, fallback legacy
...
BEGIN
  ...
  -- 1. Entitlements directs (user_id)
  ...
  WHERE user_id = v_uid AND status IN ('trialing','active') AND (ends_at IS NULL OR ends_at > now());
  -- 2. Industrial via entreprise (dirigeant ou salarié actif)
  DECLARE v_ind jsonb;
  BEGIN
