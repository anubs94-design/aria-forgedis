-- phase_h_revoke_stripe_rpc_public_access
-- Révoquer l'accès PUBLIC/anon/authenticated aux RPC Stripe SECURITY DEFINER
-- Résultat testé : anon=false, authenticated=false, service_role=true
...
-- phase_h_acquire_stripe_event_v2_returning
-- Idempotence atomique via INSERT...RETURNING : seul le worker ayant inséré reçoit acquired
-- RETURNING INTO v_inserted_id garantit l'exclusivité, ON CONFLICT verrouille la ligne existante
