...
pattern = re.compile(
    r'@app\.post\("/enroler-installation"\)\n\nasync def enroler_installation\(body: dict, request: Request\):.*?(?=\n@app\.post\("/tts"\))',
    re.S,
)
new = '''@app.post("/enroler-installation")
...
if n != 1:
    raise SystemExit(f"enrollment block replacement count={n}")
p.write_text(s2, encoding="utf-8")

(root / "tests" / "test_charlie22.py").write_text(
...
print("canonical enrollment patch applied")
